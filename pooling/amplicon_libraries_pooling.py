...
INPUT = open('/data/user_storage/pooling.csv', "r") # add your csv file
TABLE = []
for line in INPUT:
...
def run(ctx):
    ...
    # load labware
    source_plates = {
        str(i+1): ctx.load_labware('nest_96_wellplate_100ul_pcr_full_skirt', slot, 'plate ' + str(i+1)) # you may change plate type
    ...
    # load pipette
    p10 = ctx.load_instrument('p10_single', 'right', tip_racks=tipracks) # you might change pipette type, but then remember to change also tipracks
